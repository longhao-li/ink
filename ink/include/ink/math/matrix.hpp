...
struct Matrix2 {
    ...
    constexpr auto operator[](std::size_t i) noexcept -> Vector2 & { return column[i]; }
    ...
    /// @return
    ///   Reference to the specified column of elements.
    constexpr auto operator[](std::size_t i) const noexcept -> const Vector2 & { return column[i]; }
    ...
    [[nodiscard]] constexpr auto determinant() const noexcept -> float {
    ...
    [[nodiscard]] constexpr auto transposed() const noexcept -> Matrix2 {
        return {column[0][0], column[1][0], column[0][1], column[1][1]};
    }
    ...
    [[nodiscard]] constexpr auto inversed() const noexcept -> Matrix2 {
        const float det    = this->determinant();
        const float invDet = 1.0f / det;

        return {column[1][1] * invDet, column[0][1] * -invDet, column[1][0] * -invDet,
                column[0][0] * invDet};
    }

    constexpr auto operator+() const noexcept -> Matrix2 { return *this; }

    constexpr auto operator-() const noexcept -> Matrix2 { return {-column[0], -column[1]}; }
...
constexpr auto operator*(const Matrix2 &lhs, Vector2 rhs) noexcept -> Vector2 {
    return {lhs[0][0] * rhs.x + lhs[1][0] * rhs.y, lhs[0][1] * rhs.x + lhs[1][1] * rhs.y};
}

constexpr auto operator*(Vector2 lhs, const Matrix2 &rhs) noexcept -> Vector2 {
    return {lhs.x * rhs[0][0] + lhs.y * rhs[0][1], lhs.x * rhs[1][0] + lhs.y * rhs[1][1]};
}
...
struct Matrix3 {
    ...
    /// @return
    ///   Reference to the specified column of elements.
    constexpr auto operator[](std::size_t i) noexcept -> Vector3 & { return column[i]; }
    ...
    /// @return
    ///   Reference to the specified column of elements.
    constexpr auto operator[](std::size_t i) const noexcept -> const Vector3 & { return column[i]; }
    ...
    /// @return
    ///   A new matrix that represents the transposed matrix.
    [[nodiscard]] constexpr auto transposed() const noexcept -> Matrix3 {
        return {column[0][0], column[1][0], column[2][0], column[0][1], column[1][1],
    ...
    /// @return
    ///   A new matrix that represents the transposed matrix.
    [[nodiscard]] constexpr auto inversed() const noexcept -> Matrix3 {
        const float invDet = 1.0f / this->determinant();
        return Matrix3{
    ...
    [[nodiscard]] constexpr auto translated(float x, float y) noexcept -> Matrix3 {
        return {column[0][0],     column[0][1], column[0][2] + x, column[1][0], column[1][1],
    ...
    constexpr auto translated(Vector2 offset) noexcept -> Matrix3 {
        return {column[0][0], column[0][1], column[0][2] + offset.x,
                column[1][0], column[1][1], column[1][2] + offset.y,
    ...
    [[nodiscard]] auto rotated(float radian) const noexcept -> Matrix3 {
        const float s = std::sin(radian);
        const float c = std::cos(radian);

        return {column[0] * c - column[1] * s, column[0] * s + column[1] * c, column[2]};
    }
    ...
    /// @return
    ///   The scaled matrix.
    [[nodiscard]] constexpr auto scaled(float x, float y) const noexcept -> Matrix3 {
        return {column[0] * x, column[1] * y, column[2]};
    }
    ...
    /// @return
    ///   The scaled matrix.
    [[nodiscard]] constexpr auto scaled(Vector2 factor) const noexcept -> Matrix3 {
        return {column[0] * factor.x, column[1] * factor.y, column[2]};
    }
    ...
    [[nodiscard]] constexpr auto shearedX(float factor) const noexcept -> Matrix3 {
        return {column[0] + factor * column[1], column[1], column[2]};
    }
    ...
    [[nodiscard]] constexpr auto shearedY(float factor) const noexcept -> Matrix3 {
        return {column[0], column[1] + factor * column[0], column[2]};
    }

    constexpr auto operator+() const noexcept -> Matrix3 { return *this; }
...
struct Matrix4 {
    ...
    /// @return
    ///   Reference to the specified column of elements.
    constexpr auto operator[](std::size_t i) noexcept -> Vector4 & { return column[i]; }
    ...
    /// @return
    ///   Reference to the specified column of elements.
    constexpr auto operator[](std::size_t i) const noexcept -> const Vector4 & { return column[i]; }
    ...
    /// @return
    ///   A new matrix that represents the transposed matrix.
    [[nodiscard]] constexpr auto transposed() const noexcept -> Matrix4 {
        // clang-format off
        return {
    ...
    /// @return
    ///   A new matrix that represents the transposed matrix.
    [[nodiscard]] constexpr auto inversed() const noexcept -> Matrix4 {
        const float coef00 = column[2][2] * column[3][3] - column[3][2] * column[2][3];
        const float coef02 = column[1][2] * column[3][3] - column[3][2] * column[1][3];
        const float coef03 = column[1][2] * column[2][3] - column[2][2] * column[1][3];
        const float coef04 = column[2][1] * column[3][3] - column[3][1] * column[2][3];
        ...
        return {result[0] * invDet, result[1] * invDet, result[2] * invDet, result[3] * invDet};
    }
    ...
    /// @return
    ///   The translated matrix of this one.
    [[nodiscard]] constexpr auto translated(float x, float y, float z) const noexcept -> Matrix4 {
        // clang-format off
        return Matrix4{
    ...
    /// @return
    ///   The translated matrix of this one.
    [[nodiscard]] constexpr auto translated(Vector3 offset) const noexcept -> Matrix4 {
        // clang-format off
        return Matrix4{
    ...
    /// @return
    ///   The translated matrix of this one.
    [[nodiscard]] constexpr auto translated(Vector4 offset) const noexcept -> Matrix4 {
        // clang-format off
        return Matrix4{
            column[0][0], column[0][1], column[0][2], column[0][3] + offset.x,
            column[1][0], column[1][1], column[1][2], column[1][3] + offset.y,
    ...
    /// @return
    ///   Rotated matrix of this one.
    [[nodiscard]] auto rotated(Vector3 axis, float radian) const noexcept -> Matrix4 {
        const float s = std::sin(radian);
        const float c = std::cos(radian);
    ...
    [[nodiscard]] constexpr auto rotated(Quaternion quat) const noexcept -> Matrix4;
    ...
    /// @return
    ///   The scaled matrix.
    [[nodiscard]] constexpr auto scaled(float x, float y, float z) const noexcept -> Matrix4 {
        return {column[0] * x, column[1] * y, column[2] * z, column[3]};
    }
    ...
    /// @return
    ///   The scaled matrix.
    [[nodiscard]] constexpr auto scaled(Vector3 factor) const noexcept -> Matrix4 {
        return {column[0] * factor.x, column[1] * factor.y, column[2] * factor.z, column[3]};
    }

    constexpr auto operator+() const noexcept -> Matrix4 { return *this; }
...
[[nodiscard]] inline auto lookAt(Vector3 eye, Vector3 target, Vector3 up) noexcept -> Matrix4 {
...
/// @return
///   A 4x4 matrix that represents the look at transform.
[[nodiscard]] inline auto lookAt(Vector4 eye, Vector4 target, Vector4 up) noexcept -> Matrix4 {
    eye /= eye.w;
    target /= target.w;
...
/// @return
///   A 4x4 matrix that represents the look at transform.
[[nodiscard]] inline auto lookTo(Vector3 eye, Vector3 direction, Vector3 up) noexcept -> Matrix4 {
...
/// @return
///   A 4x4 matrix that represents the look at transform.
[[nodiscard]] inline auto lookTo(Vector4 eye, Vector4 direction, Vector4 up) noexcept -> Matrix4 {
    eye /= eye.w;
...
[[nodiscard]] inline auto perspective(float fovY, float aspect, float zNear, float zFar) noexcept
    -> Matrix4 {
...
perspective(float fovY, float width, float height, float zNear, float zFar) noexcept -> Matrix4 {
