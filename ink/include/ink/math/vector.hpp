...
#    pragma warning(pop)
    ...
    constexpr auto operator[](std::size_t i) noexcept -> float & { return m_arr[i]; }
    ...
    /// @return
    ///   Reference to the specified element.
    constexpr auto operator[](std::size_t i) const noexcept -> const float & { return m_arr[i]; }
    ...
    [[nodiscard]] auto length() const noexcept -> float { return std::sqrt(x * x + y * y); }
...
constexpr auto operator+(Vector2 vec) noexcept -> Vector2 { return vec; }

constexpr auto operator-(Vector2 vec) noexcept -> Vector2 { return {-vec.x, -vec.y}; }
...
[[nodiscard]] constexpr auto dot(Vector2 lhs, Vector2 rhs) noexcept -> float {
    return lhs.x * rhs.x + lhs.y * rhs.y;
}
...
[[nodiscard]] constexpr auto lerp(Vector2 start, Vector2 end, float t) noexcept -> Vector2 {
...
[[nodiscard]] constexpr auto abs(Vector2 vec) noexcept -> Vector2 {
    return Vector2{
        vec.x < 0 ? -vec.x : vec.x,
        vec.y < 0 ? -vec.y : vec.y,
    };
...
[[nodiscard]] constexpr auto min(Vector2 lhs, Vector2 rhs) noexcept -> Vector2 {
    return Vector2{
...
[[nodiscard]] constexpr auto max(Vector2 lhs, Vector2 rhs) noexcept -> Vector2 {
    return Vector2{
...
[[nodiscard]] constexpr auto clamp(Vector2 vec, Vector2 floor, Vector2 ceil) noexcept -> Vector2 {
...
#    pragma warning(pop)
    ...
    [[nodiscard]] auto length() const noexcept -> float { return std::sqrt(x * x + y * y + z * z); }
    ...
    /// @return
    ///   Normalized version of this vector.
    [[nodiscard]] auto normalized() const noexcept -> Vector3 {
        const float len    = length();
        const float invLen = 1.0f / len;
...
constexpr auto operator+(Vector3 vec) noexcept -> Vector3 { return vec; }

constexpr auto operator-(Vector3 vec) noexcept -> Vector3 { return {-vec.x, -vec.y, -vec.z}; }
...
/// @return
///   A floating value that represents the dot production.
[[nodiscard]] constexpr auto dot(Vector3 lhs, Vector3 rhs) noexcept -> float {
    return lhs.x * rhs.x + lhs.y * rhs.y + lhs.z * rhs.z;
}
...
[[nodiscard]] constexpr auto cross(Vector3 lhs, Vector3 rhs) noexcept -> Vector3 {
    return Vector3{
...
/// @return
///   The interpolation result vector.
[[nodiscard]] constexpr auto lerp(Vector3 start, Vector3 end, float t) noexcept -> Vector3 {
    return start + (end - start) * t;
}
...
/// @return
///   A new vector that contains element-wise absolute values of the original vector.
[[nodiscard]] constexpr auto abs(Vector3 vec) noexcept -> Vector3 {
    return Vector3{
        vec.x < 0 ? -vec.x : vec.x,
        vec.y < 0 ? -vec.y : vec.y,
        vec.z < 0 ? -vec.z : vec.z,
...
/// @return
///   A new vector that contains element-wise minimum values of the 2 vectors.
[[nodiscard]] constexpr auto min(Vector3 lhs, Vector3 rhs) noexcept -> Vector3 {
    return Vector3{
        lhs.x < rhs.x ? lhs.x : rhs.x,
        lhs.y < rhs.y ? lhs.y : rhs.y,
...
/// @return
///   A new vector that contains element-wise maximum values of the 2 vectors.
[[nodiscard]] constexpr auto max(Vector3 lhs, Vector3 rhs) noexcept -> Vector3 {
    return Vector3{
        lhs.x < rhs.x ? rhs.x : lhs.x,
        lhs.y < rhs.y ? rhs.y : lhs.y,
...
/// @return
///   A new vector that contains clamped elements.
[[nodiscard]] constexpr auto clamp(Vector3 vec, Vector3 floor, Vector3 ceil) noexcept -> Vector3 {
    return max(floor, min(vec, ceil));
}
...
#    pragma warning(pop)
    ...
    /// @return
    ///   Normalized version of this vector.
    [[nodiscard]] auto normalized() const noexcept -> Vector4 {
        const float len    = length();
        const float invLen = 1.0f / len;
...
constexpr auto operator+(Vector4 vec) noexcept -> Vector4 { return vec; }
...
/// @return
///   A floating value that represents the dot production.
[[nodiscard]] constexpr auto dot(Vector4 lhs, Vector4 rhs) noexcept -> float {
    return lhs.x * rhs.x + lhs.y * rhs.y + lhs.z * rhs.z + lhs.w * rhs.w;
}
...
[[nodiscard]] constexpr auto cross(Vector4 lhs, Vector4 rhs) noexcept -> Vector4 {
    return Vector4{
        lhs.y * rhs.z - lhs.z * rhs.y,
        lhs.z * rhs.x - lhs.x * rhs.z,
        lhs.x * rhs.y - lhs.y * rhs.x,
...
/// @return
///   The interpolation result vector.
[[nodiscard]] constexpr auto lerp(Vector4 start, Vector4 end, float t) noexcept -> Vector4 {
    return start + (end - start) * t;
}
...
/// @return
///   A new vector that contains element-wise absolute values of the original vector.
[[nodiscard]] constexpr auto abs(Vector4 vec) noexcept -> Vector4 {
    return Vector4{
        vec.x < 0 ? -vec.x : vec.x,
        vec.y < 0 ? -vec.y : vec.y,
        vec.z < 0 ? -vec.z : vec.z,
...
/// @return
///   A new vector that contains element-wise minimum values of the 2 vectors.
[[nodiscard]] constexpr auto min(Vector4 lhs, Vector4 rhs) noexcept -> Vector4 {
    return Vector4{
        lhs.x < rhs.x ? lhs.x : rhs.x,
        lhs.y < rhs.y ? lhs.y : rhs.y,
        lhs.z < rhs.z ? lhs.z : rhs.z,
...
/// @return
///   A new vector that contains element-wise maximum values of the 2 vectors.
[[nodiscard]] constexpr auto max(Vector4 lhs, Vector4 rhs) noexcept -> Vector4 {
    return Vector4{
        lhs.x < rhs.x ? rhs.x : lhs.x,
        lhs.y < rhs.y ? rhs.y : lhs.y,
        lhs.z < rhs.z ? rhs.z : lhs.z,
...
/// @return
///   A new vector that contains clamped elements.
[[nodiscard]] constexpr auto clamp(Vector4 vec, Vector4 floor, Vector4 ceil) noexcept -> Vector4 {
    return max(floor, min(vec, ceil));
}
