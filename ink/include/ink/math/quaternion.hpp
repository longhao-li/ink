...
struct alignas(16) Quaternion {
    ...
    [[nodiscard]] constexpr auto conjugated() const noexcept -> Quaternion {
        return {w, -x, -y, -z};
    }
    ...
    [[nodiscard]] constexpr auto inversed() const noexcept -> Quaternion {
        const float len2    = w * w + x * x + y * y + z * z;
        const float invLen2 = 1.0f / len2;
        return {w * invLen2, -x * invLen2, -y * invLen2, -z * invLen2};
    }
    ...
    [[nodiscard]] constexpr auto toMatrix() const noexcept -> Matrix4 {
...
constexpr auto operator+(Quaternion lhs, float rhs) noexcept -> Quaternion {
    return {lhs.w + rhs, lhs.x, lhs.y, lhs.z};
}
...
[[nodiscard]] constexpr auto dot(Quaternion lhs, Quaternion rhs) noexcept -> float {
    return lhs.w * rhs.w + lhs.x * rhs.x + lhs.y * rhs.y + lhs.z * rhs.z;
}
...
[[nodiscard]] inline auto nlerp(Quaternion start, Quaternion end, float t) noexcept -> Quaternion {
...
/// @return
///   The interpolation result quaternions.
[[nodiscard]] inline auto slerp(Quaternion start, Quaternion end, float t) noexcept -> Quaternion {
...
[[nodiscard]] constexpr auto Matrix4::rotated(Quaternion quat) const noexcept -> Matrix4 {
